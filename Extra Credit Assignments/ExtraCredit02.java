...
public class ExtraCredit02 {
    ...
    /** Static Method that will capitalize the start of every sentence and remove all extra white space from the string
     *
     * @param sentence The string to be edited
     * @return The edited string
     */
    public static String sentenceEditor(String sentence) {
        StringBuilder str = new StringBuilder(sentence);//create StringBuilder object to edit string
        int whiteSpaceCount = 0;//Counter for how many white spaces are between two "words" in the sentence
        //Always make the first index a capital letter
        str.setCharAt(0, sentence.toUpperCase().charAt(0));
        for(int index = str.length()-1; index >=0; index--){
            //If index is a whitespace and is the start of the space
            if(str.charAt(index) == 32 && whiteSpaceCount==0){
                //increment the whitespace counter and end iteration
                whiteSpaceCount++;
                continue;
            }
            //if anymore white spaces besides the first one
            if(str.charAt(index) == 32 && whiteSpaceCount>0){
                str.deleteCharAt(index);//remove extra whitespace
                continue;//end iteration
            }else{//if char is not a space anymore, reset the white space counter
                whiteSpaceCount = 0;
            }
            //If the index is a sentence ending mark
            if (str.charAt(index) == '.' || str.charAt(index) == '?' || str.charAt(index) == '!' ) {
                //if the two indexes ahead of the punctuation is not out of bounds and not already capitalized
                if(index + 2 < str.length() && str.charAt(index + 2) >=97)
                    //capitalize the letter
                    str.setCharAt(index+2, (char)(str.charAt(index+2) - 32));
            }
        }
        return str.toString();//return edited string
    }

    /** Method writes a table to a file that contains the frequency of chars of a string specified
     *
     * @param name The string to be traversed and analyzed
     * @param fileName The name of the file to write to
     * @throws IOException When the file is unable to be written to
     */
    public static void charCount(String name, String fileName)throws IOException{
        PrintStream output = new PrintStream(fileName);//Create PrintStream object to output to the file
        short[] asciiFrequencies = new short[94];//length of 94 is for all Letters and symbols from the ASCII table
        for(int i = 0; i < name.length(); i++){//for the strings length
            asciiFrequencies[name.charAt(i) - 32]++;//increment the ASCII frequency by one for each char index
        }
        for(int i = 0; i < asciiFrequencies.length; i++){//for the length of the ASCII Letters and Symbols Array
            //If the current ASCII index was present in the string: Print the char (frequency) to the file on a new line
            if(asciiFrequencies[i] > 0) output.printf("%c %d\n", (char) i + 32, asciiFrequencies[i]);
        }
    }
